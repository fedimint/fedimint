...
impl Amount {
    ...
    pub fn saturating_add_to(&mut self, other: Self) {
        self.msats = self.msats.saturating_add(other.msats);
    }
    ...
    pub fn saturating_mul(self, other: u64) -> Self {
        Self {
            msats: self.msats.saturating_mul(other),
        }
    }

    pub fn checked_div(self, other: Self) -> Option<u64> {
        self.msats.checked_div(other.msats)
    }

    pub fn saturating_sum(amounts: impl IntoIterator<Item = Self>) -> Self {
        amounts.into_iter().fold(Self::ZERO, Self::saturating_add)
    }
}
...
mod tests {
    ...
    fn amount_multiplication_by_scalar() {
        assert_eq!(
            Amount::from_msats(1000).saturating_mul(123),
            Amount::from_msats(123_000)
        );
    }

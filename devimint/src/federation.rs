...
use fedimint_api_client::api::{DynGlobalApi, EndpointRegistry};
...
impl Federation {
    pub async fn new(
        ...
        let endpoints = EndpointRegistry::build_from_testing_env()?.bind().await?;
        ...
        if !skip_setup && !pre_dkg {
            ...
            for peer_env_vars in peer_to_env_vars_map.values() {
                ...
                ConnectorType::default()
                    .download_from_invite_code(&endpoints, &InviteCode::from_str(&invite_code)?)
                    .await?;
            }

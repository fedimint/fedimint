...
pub async fn run_standard_load_test(
    ...
    info!("Testing run_standard_load_test");
    let output = cmd!(
        LoadTestTool,
        "--archive-dir",
    ...
    info!("Completed run_standard_load_test");
    Ok(())
}
...
pub async fn handle_command(cmd: TestCmd, common_args: CommonArgs) -> Result<()> {
    match cmd {
        ...
        TestCmd::ReconnectTest => {
            let (process_mgr, _) = setup(&common_args).await?;
            // TODO: consider different approach, just getting tests to work
            let degraded = false;
            let dev_fed = dev_fed(&process_mgr, degraded).await?;

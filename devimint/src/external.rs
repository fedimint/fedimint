...
impl Bitcoind {
    ...
    pub(crate) async fn init(client: &bitcoincore_rpc::Client, skip_setup: bool) -> Result<()> {
        ...
        if !skip_setup {
            // mine blocks
            let blocks = 101;
            let address = block_in_place(|| client.get_new_address(None, None))?.require_network();
    ...
    pub async fn send_to(&self, addr: String, amount: u64) -> Result<bitcoin::Txid> {
        ...
        let tx = self
            ...
            .await?
            .send_to_address(
                &bitcoin::Address::from_str(&addr)?.require_network(),
                amount,
            )
            .await?;
        Ok(tx)
    ...
    pub async fn get_new_address(&self) -> Result<Address> {
        ...
        let addr = block_in_place(|| client.client.get_new_address(None, None))?.require_network();
        Ok(addr)
    }

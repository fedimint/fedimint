...
impl DynGlobalApi {
    pub fn new(
        ...
        Ok(GlobalFederationApiWithCache::new(FederationApi::new(
            endpoints, peers, None, api_secret,
        ))
        .into())
    }
    pub fn new_admin(
    ...
    pub fn new_admin_setup(endpoints: EndpointRegistry, url: SafeUrl) -> anyhow::Result<Self> {
...
/// Builder for [`EndpointRegistry`]
///
/// See [`EndpointRegistry::build_from_client_env`] and similar
/// to create.
#[allow(clippy::struct_excessive_bools)] // Shut up, Clippy
pub struct EndpointRegistryBuilder {
    pub connection_overrides: BTreeMap<SafeUrl, SafeUrl>,
...
impl EndpointRegistryBuilder {
    pub async fn bind(self) -> anyhow::Result<EndpointRegistry> {
        let mut inner: BTreeMap<String, DynEndpoint> = BTreeMap::new();
...
///
/// Maps from connection URL schema to [`Endpoint`] to use to connect to it.
///
/// See [`EndpointRegistry::build_from_client_env`] and similar
/// to create.
///
/// [`EndpointRegistry::connect`] is the main entry point for making
/// mixed-networking stack connection.
#[derive(Clone, Debug)]
pub struct EndpointRegistry {
    inner: BTreeMap<String, DynEndpoint>,
...
impl ops::Deref for EndpointRegistry {
    type Target = BTreeMap<String, DynEndpoint>;
...
impl ops::DerefMut for EndpointRegistry {
...
impl EndpointRegistry {
    ...
    pub fn build_from_client_defaults() -> EndpointRegistryBuilder {
    ...
    /// usage
    pub fn build_from_server_defaults() -> EndpointRegistryBuilder {
        EndpointRegistryBuilder {
            iroh_enable: true,
            iroh_dns: None,
            iroh_pkarr_dht: true,
    ...
    /// usage
    pub fn build_from_testing_defaults() -> EndpointRegistryBuilder {
        EndpointRegistryBuilder {
            iroh_enable: true,
            iroh_dns: None,
            iroh_pkarr_dht: false,
    ...
    pub fn build_from_client_env() -> anyhow::Result<EndpointRegistryBuilder> {
    ...
    /// environment-provided overrides.
    pub fn build_from_server_env() -> anyhow::Result<EndpointRegistryBuilder> {
    ...
    /// environment-provided overrides.
    pub fn build_from_testing_env() -> anyhow::Result<EndpointRegistryBuilder> {
        let builder = Self::build_from_testing_defaults().with_env_var_overrides()?;
        Ok(builder)
    }

    /// Connect to a given `url` using matching [`Endpoint`]
    ///
...
#[async_trait]
pub trait Endpoint: Send + Sync + 'static + fmt::Debug {
    async fn connect(
        &self,
        url: &SafeUrl,
        api_secret: Option<&str>,
        ...
        fedimint_core::task::spawn(
            ...
            async move {
                ...
                while let Ok(sender) = receiver.recv().await {
                    ...
                    match endpoints.connect(&url, api_secret.as_deref()).await {
                        Ok(connection) => {

...
impl RecurringInvoiceServer {
    ...
    pub async fn get_last_used_invoice_index(
        &self,
        payment_code_id: PaymentCodeId,
    ...
    pub async fn await_next_invoice_index_generated(
        &self,
        payment_code_id: PaymentCodeId,
        previous: u64,
    ) -> Result<u64, RecurringPaymentError> {
        ...
        loop {
            let current = self.get_last_used_invoice_index(payment_code_id).await?;
            if current > previous {
                return Ok(current);
            }

...
pub struct DecryptionKeyShare(#[serde(with = "serde_impl::g1")] pub G1Affine);

#[derive(Copy, Clone, Debug, Serialize, Deserialize, Eq, PartialEq)]
pub struct AggregateDecryptionKey(#[serde(with = "serde_impl::g1")] pub G1Affine);

#[derive(Copy, Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct EphemeralPublicKey(#[serde(with = "serde_impl::g1")] pub G1Affine);

#[derive(Copy, Clone, Debug, Serialize, Deserialize, Eq, PartialEq)]
pub struct EphemeralSignature(#[serde(with = "serde_impl::g2")] pub G2Affine);
...
fn xor_with_hash(mut bytes: [u8; 32], agg_dk: &AggregateDecryptionKey) -> [u8; 32] {
    let hash = sha256::Hash::hash(agg_dk.0.to_compressed().as_ref());
...
fn hash_to_message(
    ...
    engine
        .write_all(&commitment.into_inner())
        .expect("writing to HashEngine cannot fail");

    let hash = sha256::Hash::from_engine(engine);

    G2Projective::random(&mut ChaChaRng::from_seed(hash.into_inner())).to_affine()
}

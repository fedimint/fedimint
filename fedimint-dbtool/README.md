...
```shell
fedimint-dbtool $FM_DATA_DIR/fedimintd-0/fedimintd.redb dump -- $FM_DATA_DIR/fedimintd-0 pass
```

Dump the consensus db entries of fedimintd-0
```shell
fedimint-dbtool $FM_DATA_DIR/fedimintd-0/fedimintd.redb dump -- $FM_DATA_DIR/fedimintd-0 pass consensus
```

Dump the blocks from the wallet module of fedimintd-1
```shell
fedimint-dbtool $FM_DATA_DIR/fedimintd-1/fedimintd.redb dump -- $FM_DATA_DIR/fedimintd-1 pass consensus blockhash
```

Dump the used notes from the mint module and the accepted transactions from consensus
```shell
fedimint-dbtool $FM_DATA_DIR/fedimintd-1/fedimintd.redb dump -- $FM_DATA_DIR/fedimintd-1 pass consensus,mint notenonce,acceptedtransaction
```

Dump the entire client database (client password can be anything since it doesn't require decryption)
```shell
fedimint-dbtool $FM_CLIENT_DIR/client.redb dump $FM_CLIENT_DIR clientpass client
```

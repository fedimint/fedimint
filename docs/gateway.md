...
- A payment or swap the gateway had already funded still completes, so the gateway is never left out of pocket for a contract it paid. Turning receives off waits for any payment it had already accepted to be funded, so once the command returns nothing further can enter.

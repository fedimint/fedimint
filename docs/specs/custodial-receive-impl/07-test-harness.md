...
| invalid contract audit ⇒ refund + liability; durable start/resume | `invalid_contract_refund_liability` (CP10–CP14 × duplicate wakeups; one refund txid, one open liability, ecash recovered once), `share_timeout_never_invalidates`, `invalid_refund_prepare_failure_retains_liability`, `unfinished_audit_blocks_prune`, `audit_refund_rejected_restart` (definitively reject refund, restart before and after audit observes rejection; open liability, alert, zero recovered refund ecash, retained txid/reason/evidence, no replacement submission) | G |
...
| mint-owned recovery evidence API | `mint_input_recovery_journal_restart` (spec 01 §3.6: duplicate atomic registration, untracked error, bundle rejection/per-note fallback, crash before/after child submission and output note insertion; query/subscription retain exact lineage and actual spendable amounts; unrelated same-operation transactions excluded) | G |
...
| economic threshold is issuance-only with durable accounting | `outstanding_invoices_overshoot_thresholds` (concurrent settlement exceeds inbound headroom, loss budget and max_in_flight; new issuance stops, existing debt proceeds), `loss_budget_durable_backend_scope` (duplicates, changed authenticated settlement/fee evidence after Funded adjusts the existing contribution without another funding tx, restart, prune, reset with unrelated health stop) | G |

...
2. **Set Passwords:**
	Each guardian sets two environment variables before starting fedimintd: `FM_PASSWORD_UI` (gates the admin UI login form) and `FM_PASSWORD_API` (gates admin RPCs on the public WS/iroh API). They can be the same value. `FM_PASSWORD_UI` is required: it falls back to a `password.private` file in the data dir for backwards compatibility, and if neither is set fedimintd refuses to start. Empty passwords are rejected outright: variable interpolation such as `FM_PASSWORD_UI=${PASSWORD}` can silently yield an empty string when `PASSWORD` is unset, and this must fail loudly rather than disable authentication. Passwordless mode requires the separate, explicit opt-in `FM_DANGEROUS_NO_ADMIN_UI_PASSWORD=true`, which serves the UI without a login form — meant ONLY for local deployments where `FM_BIND_UI` is guaranteed to never be exposed on a network (e.g. bound to `127.0.0.1` on a single-user machine). `FM_PASSWORD_API` is optional: unset, admin RPCs return 401 unconditionally. It never falls back to disk, since the public API is always network-reachable and must be enabled explicitly.

...
use fedimint_lnv2_common::{tweak, Bolt11InvoiceDescription};
...
const MAX_SENDABLE: u64 = 100_000_000_000;
const MIN_SENDABLE: u64 = 100_000;
...
struct Args {
    ...
    #[arg(long, default_value = "127.0.0.1:8176", env = "BIND_ADDRESS")]
    bind_address: SocketAddr,

    /// Secret for the symmetric encryption of the lnurl's payload, should
    /// have 256 bits of random entropy
...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/pay/{payload}/invoice", get(lnv2_pay_invoice))
        .with_state(state);

    info!("Starting RecurringdV2 on {}", args.bind_address);

    axum::serve(TcpListener::bind(&args.bind_address).await?, app).await?;
...
async fn lnv2_pay(
    ...
    let response = PayResponse {
        callback: format!("{}pay/{payload}/invoice", state.base_url),
...
async fn lnv2_pay_invoice(
    ...
    if params.amount < MIN_SENDABLE || params.amount > MAX_SENDABLE {

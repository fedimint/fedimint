...
rec {
  ...
  workspaceTestCovBase = { times }: craneLib.buildPackage {
    ...
    lib.concatStringsSep "\n"
      (
        lib.replicate times ''
          env RUST_BACKTRACE=1 RUST_LOG=info,timing=debug cargo nextest run --locked --workspace --all-targets --cargo-profile $CARGO_PROFILE --profile ccov --test-threads=$(($(nproc) * 2))
        ''
      ) + ''
      mkdir -p $out

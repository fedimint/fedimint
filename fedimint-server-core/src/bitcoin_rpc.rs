...
impl ServerBitcoinRpcMonitor {
    ...
    pub async fn submit_package(&self, transactions: Vec<Transaction>) -> Result<()> {
        ensure!(
            self.status_receiver.borrow().is_some(),
            "Not connected to bitcoin backend"
        );
...
pub trait IServerBitcoinRpc: Debug + Send + Sync + 'static {
    ...
    ///
    /// It is never OK to consider the success of this call as final, as the
    /// caller should be retrying broadcast periodically until it confirms the
    /// transaction was actually accepted via other means or decides that it is
    /// no longer relevant. The returned `Result` is therefore purely
    /// informational and callers are expected to log and ignore any error.
    ///

        ...
        fedimint_core::runtime::block_in_place(|| {
            match self.0.commit() {
                ...
                Err(err) => {
                    ...
                    // These are retriable - the caller should retry the entire transaction.
                    ...
                    match err.kind() {
                        ...
                        | rocksdb::ErrorKind::TimedOut => Err(anyhow::anyhow!(err)
                            .context("RocksDB optimistic transaction conflict (retriable)")),
                        _ => {
                            Err(anyhow::anyhow!(err).context("Unrecoverable RocksDB commit error"))
                        }
                    }
                }
            }

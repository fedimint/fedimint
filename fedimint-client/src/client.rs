...
use fedimint_client_module::transaction::{
    FeeQuote, TRANSACTION_SUBMISSION_MODULE_INSTANCE, TransactionBuilder, TxSubmissionStates,
...
impl Client {
    ...
    /// Computes the fee that finalizing and submitting `partial_transaction`
    /// would incur for the given `unit`, without submitting anything.
    ///
    ...
    ///
    /// The mechanism is module-agnostic — the explicit inputs/outputs come from
    /// whichever module built `partial_transaction` (mint, lightning, wallet,
    /// …), and the change is generated by the primary module for `unit`.
    ///
    /// # Panics
    /// If any of the input or output versions in the transaction builder are
    /// unknown by the respective module (only possible for transactions not
    /// built by ourselves).
    pub async fn fee_quote(
        ...
        // The primary module balances the transaction by minting change (and
        // possibly pulling in extra inputs). The amount it must balance mirrors
        // `finalize_transaction`: the input side is the gross input value, the
        // output side carries the gross output value plus every federation fee.
        let (in_amounts, out_amounts) = self.transaction_builder_get_balance(partial_transaction);
        let balance_input_amount = in_amounts.get(&unit).copied().unwrap_or_default();
        let balance_output_amount = out_amounts.get(&unit).copied().unwrap_or_default();

        // Append the change the primary module would add, exactly as
        // `finalize_transaction` does, so the breakdown below sweeps the explicit
        // items and the change uniformly. This is the only difference from a real
        // submission: the balancing runs against a non-committable transaction
        // that is dropped rather than committed.
        let mut full_transaction = partial_transaction.clone();
        if balance_input_amount != balance_output_amount {
            ...
            // notes) are discarded on drop, so this is a pure dry-run over the
            // real inventory.
            let mut dbtx = self.db.begin_transaction_nc().await;
        ...
        // Gross value and federation fees over every item — the explicit ones the
        // operation supplied plus the change the primary module added — restricted
        // to `unit`.
        ...
        let mut input_fees = Amount::ZERO;
        let mut output_fees = Amount::ZERO;

        for input in full_transaction.inputs() {
            let module = self.get_module(input.input.module_instance_id());
            let fee = module.input_fee(&input.amounts, &input.input).expect(
                "We only build transactions with input versions that are supported by the module",
            );
            gross_input += amount_of(&input.amounts);
            input_fees += amount_of(&fee);
        }

        for output in full_transaction.outputs() {
            let module = self.get_module(output.output.module_instance_id());
            let fee = module.output_fee(&output.amounts, &output.output).expect(
                "We only build transactions with output versions that are supported by the module",
            );
            gross_output += amount_of(&output.amounts);
            output_fees += amount_of(&fee);
        }

        // Net wallet gain = produced outputs − consumed inputs. The total fee is
        // everything the gross input value did not become a net gain; whatever
        // is left after the input/output fees is sub-denomination dust.
        let total = gross_input.saturating_sub(gross_output);
...
impl ClientContextIface for Client {
    ...
    async fn fee_quote(
        ...
        Client::fee_quote(self, operation_id, unit, tx_builder).await
    }

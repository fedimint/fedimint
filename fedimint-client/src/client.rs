...
impl Client {
    ...
    /// the primary module for `request.unit`.
    pub async fn fee_quote(
        ...
        // Start the totals from the explicit side; the change the primary module
        // adds below is a disjoint set of items, accumulated into these same
        // totals.
        let mut gross_input = input_amount;
        let mut gross_output = output_amount;
        let mut input_fees = input_fee;
        let mut output_fees = output_fee;
        ...
        // output side carries the gross output value plus every explicit fee.
        let balance_input_amount = input_amount;
        let balance_output_amount = output_amount + input_fee + output_fee;

    ...
    Self: Sized,
{
    ...
    ) -> RawFederationApiWithEvents<Self>;
}
    ...
    T: IRawFederationApi + MaybeSend + MaybeSync + 'static,
{
    fn with_events(
        ...
        db: Database,
        log_ordering_wakeup_tx: watch::Sender<()>,
    ) -> RawFederationApiWithEvents<T> {
        db.ensure_global().expect("Must be given global db");
        RawFederationApiWithEvents {
            inner: self,
            db,
...
///
/// Create using [`ClientRawFederationApiExt::with_events`]
#[derive(Debug)]
pub struct RawFederationApiWithEvents<I> {
    inner: I,
    db: Database,
    log_ordering_wakeup_tx: watch::Sender<()>,
}

impl<I> RawFederationApiWithEvents<I> {
...
impl<I> IRawFederationApi for RawFederationApiWithEvents<I>
where
    I: IRawFederationApi,
{

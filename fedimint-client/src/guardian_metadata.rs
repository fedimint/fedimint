...
pub(crate) async fn run_guardian_metadata_refresh_task(client_inner: Arc<Client>) {
    ...
    loop {
        if let Err(err) = {
            ...
            .await;
            store_guardian_metadata_updates_from_peers(client_inner.db(), &results).await
        } {
...
pub(crate) async fn store_guardian_metadata_updates_from_peers(
    ...
    for metadata_map in updates {
        store_guardian_metadata_updates(db, metadata_map).await;
    }
    ...
    async fn make_request(
        ...
        let result = async {
            ...
            for (peer_id, metadata) in &metadata_map {
                ...
                if let Err(e) = metadata.verify(SECP256K1, guardian_pub_key) {
                ...
                for (peer, new_metadata) in metadata_map_inner {
                    let replace_current_metadata = dbtx
                        ...
                        .is_none_or(|current_metadata| {
                            ...
                            if let (Ok(current_parsed), Ok(new_parsed)) = (
                                current_metadata.guardian_metadata(),
                                new_metadata.guardian_metadata(),
                            ) {
                                new_parsed.timestamp_secs > current_parsed.timestamp_secs
                            } else {
                                // If we can't parse, replace anyway
                                true
                            }
                        });

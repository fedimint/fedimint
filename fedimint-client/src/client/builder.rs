...
use fedimint_api_client::api::{
    ApiVersionSet, DynGlobalApi, EndpointRegistry, FederationApi, FederationApiExt as _,
};
...
impl ClientBuilder {
    ...
    pub async fn preview_with_existing_config(
        ...
        self.preview_inner(endpoints, config, api_secret, None, None)
            .await
    }
    ...
    async fn build_stopped(
        ...
        let api = match self.admin_creds.as_ref() {
            ...
            .with_cache()
            .into(),
            None => FederationApi::new(endpoints.clone(), peer_urls, None, api_secret.as_deref())
                .with_client_ext(db.clone(), log_ordering_wakeup_tx.clone())
                .with_request_hook(&request_hook)
                .with_cache()
                .into(),

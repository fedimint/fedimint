...
/// Breakdown of the fee finalizing a [`TransactionBuilder`] would incur, as
/// computed by `Client::fee_quote` (a dry-run of the same balancing the real
/// submission performs).
///
/// This is module-agnostic: the explicit inputs/outputs come from whichever
/// module built the partial transaction (mint, lightning, wallet, …) and the
/// change is generated by the primary module. The quote is point-in-time: it
/// depends on the client's current inventory and can move as funds change.
///

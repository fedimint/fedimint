...
pub async fn start_guardian_metadata_service(
    ...
    insert_signed_guardian_metadata_if_not_present(db, cfg).await;
...
async fn insert_signed_guardian_metadata_if_not_present(db: &Database, cfg: &ServerConfig) {
...
/// Returns a list of all peers and their respective API URLs from
/// configuration. Note: This doesn't override with guardian metadata yet - that
/// would require additional implementation.
pub async fn get_api_urls(
    _db: &Database,
    cfg: &ServerConfigConsensus,
) -> BTreeMap<PeerId, SafeUrl> {
    // For now, just return the configured API URLs
    // TODO: Consider guardian metadata when determining API URLs
    cfg.api_endpoints()
        .iter()

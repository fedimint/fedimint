...
use fedimint_core::module::{
    ApiRequestErased, CORE_CONSENSUS_VERSION, CoreConsensusVersion,
    DYNAMIC_FEES_CORE_CONSENSUS_VERSION, ModuleConsensusVersion, SerdeModuleEncoding,
};
...
use crate::consensus::db::{
    ...
    ModuleConsensusVersionVoteKey, ModuleFeeConsensusScheduleKey, ModuleFeeConsensusSchedulePrefix,
...
impl ConsensusEngine {
    ...
    async fn process_consensus_item_with_db_transaction(
        ...
        match consensus_item {
            ...
            ConsensusItem::CoreConsensusVersion(vote) => {
                // The DKG time config version is the floor for every peer's vote, hence
                // per-peer votes strictly increase and the active core consensus version
                // can never decrease.
                let current_vote = dbtx
                ...
                if vote <= current_vote {
                    bail!("Core consensus version vote is redundant");
                }
            ...
            ConsensusItem::Transaction(transaction) => {
                ...
                let active_module_consensus_versions = self
                    ...
                    .await?;
                let active_core_consensus_version = self.active_core_consensus_version(dbtx).await;

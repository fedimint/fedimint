...
use fedimint_core::endpoint_constants::{
    SUPPORTED_CORE_CONSENSUS_VERSION_ENDPOINT, SUPPORTED_MODULE_CONSENSUS_VERSION_ENDPOINT,
};
...
use fedimint_core::module::{
    ApiAuth, ApiEndpoint, ApiError, ApiMethod, ApiRequestErased, CORE_CONSENSUS_VERSION,
    CoreConsensusVersion, FEDIMINT_API_ALPN, IrohApiRequest, ModuleConsensusVersion,
};
...
use crate::consensus::db::{
    CoreConsensusVersionVotingActivationKey, ModuleConsensusVersionVotingActivationKey,
    active_core_consensus_version, active_module_consensus_version,
};
...
fn submit_core_ci_proposals(task_group: &TaskGroup, context: CoreCiProposalContext) {
    let CoreCiProposalContext {
        db,
        federation_api,
        our_peer_id,
        num_peers,
    ...
    let vote_check_interval = if is_running_in_test_env() {
    ...
    task_group.spawn("core_citem_proposals", move |task_handle| async move {
        let mut last_vote_check = None;
        while !task_handle.is_shutting_down() {
            let active_core_consensus_version = active_core_consensus_version(
            ...
            let check_vote =
                last_vote_check.is_none_or(|last: Instant| last.elapsed() >= vote_check_interval);
            if check_vote {
                last_vote_check = Some(Instant::now());
            }

            if let Some(vote_version) = core_consensus_version_vote(
                &db,
                &federation_api,
                our_peer_id,
                active_core_consensus_version,
                check_vote,
                automatic_voting_disabled,
            )
            .await
                && submission_sender
                    .send(ConsensusItem::CoreConsensusVersion(vote_version))
                    .await
                    .is_err()
            {
                warn!(
                    target: LOG_CONSENSUS,
                    "Unable to submit core consensus version vote proposal via channel"
                );
            }
...
///
/// Unlike module consensus version votes, a core consensus version vote is
/// carried in a consensus item variant older binaries do not know about, and
/// they panic on unknown consensus items. Both the automatic and the manual
/// vote are therefore only proposed once every peer has reported supporting the
/// version we are about to vote for.
async fn core_consensus_version_vote(
    db: &Database,
    federation_api: &DynGlobalApi,
    our_peer_id: PeerId,
    active_version: CoreConsensusVersion,
    check_vote: bool,
    automatic_voting_disabled: bool,
) -> Option<CoreConsensusVersion> {
    if !check_vote {
        return None;
    }
    ...
    let manual_vote = db
        .begin_transaction_nc()
        .await
        .get_value(&CoreConsensusVersionVotingActivationKey)
        .await
        .filter(|version| active_version < *version);

    let automatic_vote_due = !automatic_voting_disabled && active_version < CORE_CONSENSUS_VERSION;

    if manual_vote.is_none() && !automatic_vote_due {
        return None;
    }

    let all_peers_supported_version =
        all_peers_supported_core_consensus_version(federation_api, our_peer_id).await?;

    // A manual vote may never exceed what all peers support, and without one we
    // vote for the highest version all peers support.
    let vote = manual_vote.map_or(all_peers_supported_version, |manual_vote| {
        manual_vote.min(all_peers_supported_version)
    });

    (active_version < vote).then_some(vote)
}

/// Returns the highest core consensus version supported by every peer, or
/// `None` if not all peers have reported their supported version.
async fn all_peers_supported_core_consensus_version(
    federation_api: &DynGlobalApi,
    our_peer_id: PeerId,
) -> Option<CoreConsensusVersion> {
    let request_futures = federation_api.all_peers().iter().filter_map(|&peer| {
        ...
        Some(async move {
            federation_api
                .request_single_peer::<CoreConsensusVersion>(
                    SUPPORTED_CORE_CONSENSUS_VERSION_ENDPOINT.to_owned(),
                ...
                .inspect_err(|err| {
                    warn!(
                        ...
                        peer = %peer,
                        err = %err.fmt_compact(),
                        "Failed to fetch supported core consensus version from peer"
                    );
                })
                .ok()
        })
    ...
    let mut supported_versions = join_all(request_futures)
        ...
        .into_iter()
        .flatten()
        .chain(std::iter::once(CORE_CONSENSUS_VERSION))
        .collect::<Vec<_>>();
    ...
    if supported_versions.len() != federation_api.all_peers().len() {
        trace!(
            ...
            "Not all peers have reported their supported core consensus version yet"
        );
        return None;
    }
    ...
    debug!(
        ...
        "Fetched supported core consensus versions from peers"
    );

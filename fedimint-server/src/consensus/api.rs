...
impl ConsensusApi {
    ...
    async fn submit_guardian_metadata(
        ...
        if let Err(e) = metadata.verify(SECP256K1, peer_key) {
        ...
        if let Some(existing_metadata) = dbtx.get_value(&GuardianMetadataKey(peer_id)).await {
            ...
            // the caller to stop submitting the value if they are in a retry loop.
            if existing_metadata.json_bytes == metadata.json_bytes {
                return Ok(());
            }

            // Only update if the new metadata has a newer timestamp
            let existing_parsed = existing_metadata
                .guardian_metadata()
                .map_err(|e| ApiError::bad_request(format!("Existing metadata invalid: {e}")))?;
            let new_parsed = metadata
                .guardian_metadata()
                .map_err(|e| ApiError::bad_request(format!("New metadata invalid: {e}")))?;

            if new_parsed.timestamp_secs <= existing_parsed.timestamp_secs {
                return Err(ApiError::bad_request(

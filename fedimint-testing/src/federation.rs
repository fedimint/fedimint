...
use fedimint_api_client::api::{DynGlobalApi, EndpointRegistry, FederationApiExt};
...
impl FederationTest {
    ...
    pub async fn new_admin_api(&self, peer_id: PeerId) -> anyhow::Result<DynGlobalApi> {
        ...
        DynGlobalApi::new_admin(
            EndpointRegistry::build_from_testing_env()?.bind().await?,
            peer_id,
    ...
    pub async fn new_client_with(
        ...
        client_builder
            .preview_with_existing_config(self.endpoints.clone(), client_config, None)
            .await
...
impl FederationTestBuilder {
    ...
    pub async fn build(self) -> FederationTest {
        ...
        for (peer_id, cfg) in configs.clone() {
            ...
            task_group.spawn("fedimintd", move |_| async move {
                Box::pin(consensus::run(
                    EndpointRegistry::build_from_testing_env()
                        .unwrap()
                        .bind()
                        .await
                        .unwrap(),
        ...
        for (peer_id, config) in configs.clone() {
            ...
            let endpoints = EndpointRegistry::build_from_testing_env()
                .unwrap()
                .bind()
                .await
                .unwrap();
        ...
        FederationTest {
            ...
            endpoints: EndpointRegistry::build_from_testing_env()

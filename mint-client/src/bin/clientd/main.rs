...
use tide::Request;
use tide::Response;
use mint_client::{MintClient, ResBody, CoinsByTier, parse_coins, serialize_coins};
...
async fn info(req: Request<State>) -> tide::Result {
    ...
    let body= Body::from_json(&ResBody::build_info(mint_client.coins(), cfd)).expect("encoding error");
    Ok(body.into())
}
...
async fn pending(req : Request<State>) -> tide::Result {
    let mint_client = &req.state().mint_client;
    let cfd = mint_client.fetch_active_issuances();
    let body= Body::from_json(&ResBody::build_pending(cfd)).expect("encoding error");
    Ok(body.into())
}
...
async fn spend(mut req: Request<State>) -> tide::Result {
    ...
    let body = Body::from_json(&ResBody::build_spend(token)).unwrap();
    Ok(body.into())
}
...
async fn reissue_validate(mut req: Request<State>) -> tide::Result {
    ...
    let body = Body::from_json(&ResBody::build_reissue(out_point, status)).unwrap();

...
use std::hash::Hash;
use std::sync::Arc;
use tbs::{combine_valid_shares, sign_blinded_msg, verify_blind_share, Aggregatable, AggregatePublicKey, PublicKeyShare, SecretKeyShare};
use thiserror::Error;
...
impl Mint {
    ...
    fn blind_sign(&self, output: Coins<BlindToken>) -> Result<PartialSigResponse, MintError> {
        return Ok(PartialSigResponse(output
...
mod test {
    ...
    fn build_configs() -> (Vec<MintConfig>, MintClientConfig) {
        ...
        let (mint_cfg, client_cfg) =
            MintConfig::trusted_dealer_gen(&peers, THRESHOLD, &[Amount::from_sat(1)], OsRng::new().unwrap());
    ...
    fn build_mints() -> (AggregatePublicKey, Vec<Mint>) {
        ...
        let mints = mint_cfg
            .into_iter()
            .map(|config| {
                Mint::new(config, MINTS - THRESHOLD, Arc::new(MemDatabase::new()))
            })
            .collect::<Vec<_>>();

        let agg_pk = client_cfg.tbs_pks.keys.get(&Amount::from_sat(1)).unwrap().clone();
    ...
    fn test_issuance() {
        ...
        let blind_tokens = Coins {
            coins: vec![(Amount::from_sat(1), vec![BlindToken(bmsg), BlindToken(bmsg)])]
                .into_iter()
                .collect(),
        };
        ...
        assert_eq!(bsig_res, Err(CombineError::MultiplePeerContributions(PeerId::from(0), 2)));
        assert!(errors.0.is_empty());
        ...
        assert!(bsig_res.is_ok());
        assert!(errors.0.contains(&(PeerId::from(2), PeerErrorType::InvalidSignature)));
        ...
        assert!(bsig_res.is_ok());
        assert!(errors.0.contains(&(PeerId::from(3), PeerErrorType::DifferentNonce)));
    }
    ...
    fn test_new_panic_without_own_pub_key() {
        ...
        }, THRESHOLD, Arc::new(MemDatabase::new()));
    }
}

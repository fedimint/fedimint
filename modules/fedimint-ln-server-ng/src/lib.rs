...
use fedimint_ln_common_ng::config::{
...
use fedimint_ln_common_ng::{
    LightningCommonInit, LightningConsensusItem, LightningInput, LightningInputError,
...
impl ServerModuleInit for LightningInit {
    ...
    async fn distributed_gen(
        ...
        let server = LightningConfig {
            ...
            consensus: LightningConfigConsensus {
                tpe_agg_pk: tpe::AggregatePublicKey(evaluate_polynomial_g1(
                    &poly_g1,
                    &Scalar::zero(),
                )),
                tpe_pks: peers
...
impl ServerModule for Lightning {
    ...
    async fn process_input<'a, 'b, 'c>(
        ...
        let pub_key = match &input.witness {
            Witness::Outgoing(contract_key, outgoing_witness) => {
                if let Some(preimage) = dbtx.get_value(&contract_key.preimage_key()).await {
                    return Err(LightningInputError::AlreadyClaimed(preimage));
                }

                let contract = dbtx
                    .remove_entry(contract_key)
                    .await
                    .ok_or(LightningInputError::UnknownContract)?;

                if input.amount != contract.amount {
                    return Err(LightningInputError::InvalidAmount);
                }
                ...
                match outgoing_witness {
                    OutgoingWitness::Claim(preimage) => {
                        ...
                        dbtx.insert_new_entry(&contract_key.preimage_key(), preimage)
                            .await;
            ...
            Witness::Incoming(contract_key, agg_decryption_key) => {
                let contract = dbtx
                    .remove_entry(contract_key)
                    .await
                    .ok_or(LightningInputError::UnknownContract)?;

                if input.amount != contract.commitment.amount {
                    return Err(LightningInputError::InvalidAmount);
                }
    ...
    async fn process_output<'a, 'b>(
        ...
        let outcome = match output {
            LightningOutput::Outgoing(contract) => {
                if dbtx
                    .insert_entry(&contract.contract_key(), contract)
                    .await
                    .is_some()
                {
    ...
    fn api_endpoints(&self) -> Vec<ApiEndpoint<Self>> {
        vec![
            ...
            api_endpoint! {
                ...
                ApiVersion::new(0, 0),
                async |module: &Lightning, context, params: (IncomingContractKey, u64) | -> Option<PublicKey> {
                    let db = context.db();
            ...
            api_endpoint! {
                ...
                ApiVersion::new(0, 0),
                async |module: &Lightning, context, params: (PreimageKey, u64)| -> Option<[u8; 32]> {
                    let db = context.db();
            ...
            api_endpoint! {
                OUTGOING_CONTRACT_EXPIRATION_ENDPOINT,
                ApiVersion::new(0, 0),
                async |module: &Lightning, context, contract_key: OutgoingContractKey| -> Option<u64> {
                    let db = context.db();

                    Ok(module.outgoing_contract_expiration(db, contract_key).await)
                }
            },
        ]
    }
...
impl Lightning {
    ...
    async fn await_incoming_contract(
        ...
        loop {
            timeout(Duration::from_secs(10), db.wait_key_exists(&contract_key))
                .await
                .ok();
            ...
            if let Some(contract) = dbtx.get_value(&contract_key).await {
                return Some(contract.commitment.refund_pk);
            }
    ...
    async fn await_preimage(
        ...
        loop {
            timeout(Duration::from_secs(10), db.wait_key_exists(&preimage_key))
                .await
                .ok();
            ...
            if let Some(preimage) = dbtx.get_value(&preimage_key).await {
    ...
    async fn outgoing_contract_expiration(
        ...
        let contract = dbtx.get_value(&contract_key).await?;

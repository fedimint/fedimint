...
use bitcoin::{
    Address, AddressType, Amount, BlockHash, EcdsaSig, EcdsaSighashType, Network, OutPoint, Script,
...
use fedimint_api::module::InputMeta;
...
#[async_trait(?Send)]
impl ServerModulePlugin for L1ServerModule {
    type Common = L1ModuleCommon;
    type Input = Box<PegInProof>;
    type Output = PegOut;
    type PendingOutput = L1PendingOutput;
    type SpendableOutput = L1SpendableOutput;
    type OutputOutcome = PegOutOutcome;
    type ConsensusItem = WalletConsensusItem;
    type VerificationCache = ();

    fn init(&self, backend: &mut dyn InitHandle) {
        backend
            .register_typed_endpoint("/block_height", |_params: (), _ctx| {
                Box::pin(async move { Ok(module.consensus_height().unwrap_or(0)) })
            })
            .register_typed_endpoint("/peg_out_fees", |params: (Address, u64), _ctx| {
                Box::pin(async move {

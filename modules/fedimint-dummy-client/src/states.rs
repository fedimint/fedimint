...
pub enum DummyStateMachine {
    Input(Amount, AmountUnit, TransactionId, OperationId),
    Output(Amount, AmountUnit, TransactionId, OperationId),
    InputDone(OperationId),
    OutputDone(Amount, AmountUnit, TransactionId, OperationId),
    Refund(OperationId),
    Unreachable(OperationId, Amount),
}
...
impl State for DummyStateMachine {
    ...
    fn transitions(
        ...
        match self.clone() {
            DummyStateMachine::Input(amount, unit, txid, id) => vec![StateTransition::new(
                ...
                move |dbtx, res, _state: Self| match res {
                    ...
                    Err(_) => Box::pin(async move {
                        add_funds(amount, unit, dbtx.module_tx()).await;
                        DummyStateMachine::Refund(id)
                    }),
                },
            )],
            DummyStateMachine::Output(amount, unit, txid, id) => vec![StateTransition::new(
                await_tx_accepted(global_context.clone(), txid),
                move |dbtx, res, _state: Self| match res {
                    // output accepted, add funds
                    Ok(()) => Box::pin(async move {
                        add_funds(amount, unit, dbtx.module_tx()).await;
                        DummyStateMachine::OutputDone(amount, unit, txid, id)
                    }),
    ...
    fn operation_id(&self) -> OperationId {
        match self {
            DummyStateMachine::Input(_, _, _, id)
            | DummyStateMachine::Output(_, _, _, id)
            | DummyStateMachine::InputDone(id)
            | DummyStateMachine::OutputDone(_, _, _, id)
            | DummyStateMachine::Refund(id)

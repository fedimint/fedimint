...
async fn test_lnurl_pay(dev_fed: &DevJitFed) -> anyhow::Result<()> {
    ...
    let recurringdv2_addr = dev_fed.recurringdv2().await?.addr.clone();
    ...
    for (gw_send, gw_receive) in gateway_matrix {
        ...
        let lnurl_a = generate_lnurl(&client_a, &recurringdv2_addr, &gw_receive.addr).await?;
        let lnurl_b = generate_lnurl(&client_b, &recurringdv2_addr, &gw_receive.addr).await?;

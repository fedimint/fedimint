...
async fn main() -> anyhow::Result<()> {
    ...
    devimint::run_devfed_test()
        .call(move |dev_fed, _process_mgr| async move {
            ...
            // exercising the descriptor we were asked for. Older clients
            // cannot decode the new `WalletDescriptor` variants in the client
            // config at all, so `join-federation` fails outright.
            if descriptor != WalletDescriptorKind::Wsh
                && (fedimintd_version < *VERSION_0_13_0_ALPHA
                    || fedimint_cli_version < *VERSION_0_13_0_ALPHA)
            {
                info!(
                    %fedimintd_version,

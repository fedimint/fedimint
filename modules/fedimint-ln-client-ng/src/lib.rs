...
use crate::receive_sm::{Pending, ReceiveSMState, ReceiveStateMachine};
use crate::send_sm::{SendSMState, SendStateMachine};
...
/// Default feerate for outgoing lightning payments
const SEND_FEERATE_MILLIONTHS_DEFAULT: u64 = 10_000;

/// Default maximum feerate for incoming lightning payments
const RECEIVE_FEERATE_MAX_MILLIONTHS_DEFAULT: u64 = 10_000;
...
pub struct PaymentFees {
    pub send_feerate_millionths: u64,
    pub receive_feerate_millionths: u64,
}

impl PaymentFees {
    pub fn incoming_contract_amount(&self, msats: u64) -> Amount {
        Amount::from_msats(msats.saturating_sub(self.receive_fees(msats)))
    }

    fn receive_fees(&self, msats: u64) -> u64 {
        msats
            .saturating_mul(self.receive_feerate_millionths)
...
impl ModuleInit for LightningClientInit {
    ...
    async fn dump_database(
        ...
        let ln_client_items: BTreeMap<String, Box<dyn erased_serde::Serialize + Send>> =
            BTreeMap::new();
...
impl LightningClientModule {
    ...
    pub async fn send(
        ...
        self.send_internal(
            gateway_api,
            invoice,
            SEND_FEERATE_MILLIONTHS_DEFAULT,
    ...
    pub async fn send_internal(
        ...
        // we account for fees along the lightning route
        let min_feerate_millionths = 2 * payment_info.payment_fees.send_feerate_millionths;

        ensure!(
            feerate_millionths >= min_feerate_millionths,
            "The minimum possible feerate is {min_feerate_millionths}"
        );

        // we account for cltv expiry deltas along the lightning route
        ...
        let contract_msats = invoice_msats + ((invoice_msats * feerate_millionths) / 1_000_000);

        let contract = OutgoingContract {
            payment_hash: *invoice.payment_hash(),
            amount: Amount::from_msats(contract_msats),
        ...
        let client_output = ClientOutput::<LightningOutput, LightningClientStateMachines> {
            ...
            state_machines: Arc::new(move |txid, _| {
                vec![LightningClientStateMachines::Send(SendStateMachine {
                    ...
                    state: SendSMState::Funding(send_sm::Funding {
                        txid,
                    ...
                    if let Some(LightningClientStateMachines::Send(state)) = stream.next().await {
                        match state.state {
                            ...
                            SendSMState::Success(preimage) => {
                                ...
                                assert!(state.contract.verify_preimage(&preimage));
                                ...
                                match client_ctx.await_primary_module_outputs(operation_id, out_points.clone()).await {
                                    ...
                                    Err(..) => {
                                        ...
                                        if let Some(preimage) = module_api.await_preimage(
                                            &state.contract.contract_key().preimage_key(),
                                            0
                                        ).await {
                                            if state.contract.verify_preimage(&preimage) {
                                                yield SendState::Success(preimage);
                                                return;
                                            }
                                        }
    ...
    pub async fn receive(
        ...
        self.receive_internal(
            ...
            String::new(),
            RECEIVE_FEERATE_MAX_MILLIONTHS_DEFAULT,
        )
        .await
    }
    ...
    pub async fn create_contract_and_fetch_invoice_internal(
        ...
        let payment_info = self.payment_info(gateway_api.clone()).await?;
            ...
            max_feerate_millionths >= payment_info.payment_fees.send_feerate_millionths,
            "The maximum possible feerate is {max_feerate_millionths}"
        );

        let contract_amount = payment_info
            .payment_fees
            .incoming_contract_amount(invoice_amount.msats);
    ...
    pub async fn receive_external_contract(
        ...
        let receive_sm = LightningClientStateMachines::Receive(ReceiveStateMachine {
            operation_id,
            contract,
            state: ReceiveSMState::Pending(Pending {
                claim_keypair,
                agg_decryption_key,
            }),
        });

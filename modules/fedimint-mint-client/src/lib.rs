...
use fedimint_core::db::{
    AutocommitError, Database, DatabaseVersion, IReadDatabaseTransactionOpsTyped,
...
impl ClientModuleInit for MintClientInit {
    ...
    async fn recover(
        ...
        let mut dbtx = args.db().begin_read_transaction().await;
        ...
        self.client_ctx
            ...
            .map_err(|e| match e {
                AutocommitError::CommitFailed {
                    last_error,
                    attempts,
                } => anyhow::anyhow!("Commit failed after {attempts} attempts: {last_error}"),
                AutocommitError::ClosureError { error, .. } => error,
            })
    }

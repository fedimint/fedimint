...
impl FromStr for OOBNotes {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let oob_notes: OOBNotes = if let Ok(oob_notes) = base32::decode_oob(&s) {
            oob_notes
        } else if let Ok(oob_notes) = Decodable::consensus_decode_whole(
            ...
            .context("Could not decode OOB notes")?
        };

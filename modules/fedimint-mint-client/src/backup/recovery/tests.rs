...
use fedimint_client::secret::DeriveableSecretClientExt;
...
use crate::output::{MultiNoteIssuanceRequest, NoteIssuanceRequest};
...
impl MicroMintClient {
    fn from_short_seed(id: u8) -> Self {
        ...
        Self::new(root_secret.derive_module_secret(0))
    }
    ...
    fn generate_pending_note(&mut self, amount: Amount) -> (NoteIssuanceRequest, BlindNonce) {
        ...
        NoteIssuanceRequest::new(
            secp256k1::SECP256K1,
            MintClientModule::new_note_secret_static(&self.secret, amount, note_idx),
        )
    }
...
fn sanity_check_recovery_fresh_backup() {
    ...
    // nothing.
    let mut tracker = MintRestoreInProgressState::from_backup(
        10,
        empty_backup_c1,
        gap_limit,
        fed.tbs_pks.clone(),

...
use crate::{
    ...
    SelectNotesWithExactAmount, SpendOOBRefund, SpendOOBState,
};
...
impl MintClientModule {
    ...
    pub async fn subscribe_reissue_external_notes_uniffi(
        ...
        callback: Box<dyn ReissueNotesStateCallback>,
    ) -> Result<()> {
    ...
    #[uniffi::method(name = "subscribe_spend_notes")]
    pub async fn subscribe_spend_notes_uniffi(
        &self,
        operation_id: OperationId,
        callback: Box<dyn SpendNotesStateCallback>,
    ) -> Result<()> {
        let client_ctx = self.client_ctx.clone();
        let mint = client_ctx.self_ref();
...
pub trait ReissueNotesStateCallback: Send + Sync {
    fn on_state(&self, operation_id: OperationId, state: ReissueExternalNotesState);
}

#[cfg(feature = "uniffi")]
#[uniffi::export(callback_interface)]
pub trait SpendNotesStateCallback: Send + Sync {
    fn on_state(&self, operation_id: OperationId, state: SpendOOBState);
}

...
use crate::db::{
    ...
    IncomingContractKey, IncomingContractOutpointKey, IncomingContractOutpointPrefix,
    IncomingContractPrefix, IncomingContractStreamIndexKey, IncomingContractStreamKey,
    ...
    async fn process_input<'a, 'b, 'c>(
        ...
        let (pub_key, amount) = match input.ensure_v0_ref()? {
            ...
            LightningInputV0::Incoming(outpoint, agg_decryption_key) => {
                ...
                if let Some(index) = dbtx.get_value(&IncomingContractIndexKey(*outpoint)).await {

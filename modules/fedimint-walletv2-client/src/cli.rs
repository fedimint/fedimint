...
enum InfoOpts {
    ...
    TransactionChain { n_transactions: usize },
...
pub(crate) async fn handle_cli_command(
    ...
    let value = match opts {
        Opts::Info(subcommand) => match subcommand {
            ...
            InfoOpts::PendingTransactionChain => {
                json(wallet.info().pending_transaction_chain().await?)
            }
            ...
            InfoOpts::TransactionChain { n_transactions } => {
                json(wallet.info().transaction_chain(n_transactions).await?)
            }
        },

...
impl LightningClientModule {
    ...
    /// Generate an lnurl for the client. You can optionally specify the
    /// recurringd server to use, otherwise we default to <https://recurringdv2.fedimint.org>.
    /// You can optionally specify a gateway to use for testing purposes.
    pub async fn generate_lnurl(
        &self,
        recurringd: Option<SafeUrl>,
        gateway: Option<SafeUrl>,
    ) -> Result<String, RegisterLnurlError> {
        let recurringd = match recurringd {
            Some(recurringd) => recurringd,
            None => SafeUrl::from_str("https://recurringdv2.fedimint.org").unwrap(),
        };

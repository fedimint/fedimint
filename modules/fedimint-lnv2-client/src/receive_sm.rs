...
use fedimint_lnv2_common::contracts::{IncomingContract, fee_from_expiry_or_fee};
...
impl ReceiveStateMachine {
    ...
    async fn await_incoming_contract(
        ...
        global_context
            .module_api()
            .await_incoming_contract(&contract.contract_id(), contract.commitment.expiry_or_fee)
            .await
    }
    ...
    async fn transition_incoming_contract(
            ...
            .map(|operation| operation.meta::<LightningOperationMeta>())
        {
            ...
            _ => Amount::from_msats(fee_from_expiry_or_fee(
                old_state.common.contract.commitment.expiry_or_fee,
            )),
        };

...
use fedimint_client_module::module::{PrimaryModulePriority, PrimaryModuleSupport};
...
impl MintClientModule {
    ...
    /// Recover the unspent ecash set by downloading it from the federation
    /// in an efficient way.
    pub async fn recover_ecash(&self) -> anyhow::Result<Vec<NoteIssuanceRequest>> {
        let tweak_grind_seed = self.root_secret.to_random_bytes::<32>();

        let mut recovery_stream = Box::pin(self.download_recovery_slices().await);

        let mut requests: BTreeMap<hash160::Hash, NoteIssuanceRequest> = BTreeMap::new();

        let mut nonces: BTreeSet<hash160::Hash> = BTreeSet::new();

        while let Some(item) = recovery_stream.next().await {
            match item {
                RecoveryItem::Output {
                    denomination,
                    nonce_hash,
                    ...
                    if !issuance::check_nonce(tweak, &self.root_secret, nonce_hash) {
                        continue;
                    }

                    let nonce_hash = issuance::nonce(tweak, &self.root_secret).consensus_hash();

                    // Our mint outputs can theoretically be resubmitted by someone else.
                    if !nonces.insert(nonce_hash) {
                        continue;
                    }

                    requests.insert(nonce_hash, NoteIssuanceRequest::new(denomination, tweak));
                }
                RecoveryItem::Input { nonce_hash } => {
                    requests.remove(&nonce_hash);
                }
            }
        }
    ...
    async fn download_recovery_slices(&self) -> impl Stream<Item = RecoveryItem> {
        ...
        let peer_selector = PeerSelector::new(global_api.all_peers().clone());

        // Fetch the total count
        let count: u64 = module_api
            .fetch_recovery_count()
            .await
            .expect("Failed to fetch recovery count");

        // Convert iterator to stream and process in parallel
        futures::stream::iter(0..count.div_ceil(SLICE_SIZE))
            .map(move |i| {
                Self::download_single_recovery_slice(
                    module_api.clone(),
                    peer_selector.clone(),
                    i * SLICE_SIZE,
                    std::cmp::min((i + 1) * SLICE_SIZE, count),
                )
            })
    ...
    async fn download_single_recovery_slice(

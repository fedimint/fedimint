...
pub struct MintRecoveryState {
    requests: BTreeMap<PublicKey, (OutPoint, NoteIssuanceRequest)>,
}
    ...
    async fn handle_output(
        ...
        match output {
            MintOutput::V0(output) => {
                if let Some(request) = NoteIssuanceRequest::recover(output, &self.root_secret) {
                    self.state.requests.insert(
                        request.keypair(&self.root_secret).public_key(),
                        (outpoint, request),
                    );
                }
            }

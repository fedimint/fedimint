...
use fedimint_ln_client_ng::{LightningClientInit, LightningClientModule, ReceiveState, SendState};
use fedimint_ln_common_ng::config::LightningGenParams;
use fedimint_ln_server_ng::LightningInit;
...
fn fixtures() -> Fixtures {
    ...
    let fixtures = fixtures.with_module(LightningClientInit, LightningInit, ln_params);

    // we attach the legacy lightning module to the client for gateway registration
    let ln_params =
        fedimint_ln_common::config::LightningGenParams::regtest(fixtures.bitcoin_server());

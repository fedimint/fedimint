...
impl ClientModuleInit for WalletClientInit {
    ...
    /// addresses (or index 0 one) until MAX_GAP unused ones.
    ///
    /// Notably does not persist the progress of addresses being querried,
    ...
    async fn recover(
        ...
        let start_tweak_idx = match _snapshot {
        ...
        let mut tweak_idx = start_tweak_idx;
        ...
        let new_start_tweak_idx = loop {
            let gap_since_last_used = tweak_idx - last_used_idx.unwrap_or_default();

            args.update_recovery_progress(RecoveryProgress {
                complete: u32::try_from(tweak_idx.0).unwrap_or(u32::MAX),
                total: u32::try_from(tweak_idx.0.saturating_add(MAX_GAP)).unwrap_or(u32::MAX),
            });
            ...
            let (script, address, _tweak_key, _operation_id) = data.derive_peg_in_script(tweak_idx);
            btc_rpc.watch_script_history(&script).await?;
            let history = btc_rpc.get_script_history(&script).await?;

            debug!(target: LOG_CLIENT_MODULE_WALLET, %tweak_idx, %address, history_len=history.len(), "Checked address");
            if !history.is_empty() {
                last_used_idx = Some(tweak_idx);
                tweak_idxes_with_pegins.insert(tweak_idx);
            }

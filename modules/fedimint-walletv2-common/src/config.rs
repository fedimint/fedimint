...
use fedimint_core::{plugin_types_trait_impl_config, weight_to_vbytes, Amount, PeerId};
...
impl WalletConfigConsensus {
    ...
    pub fn new(
        ...
        let change_witness_weight = descriptor(&bitcoin_pks, &42_u64.consensus_hash())
...
fn test_fee_consensus() {
    ...
    assert_eq!(
        ...
        Amount::from_msats(10) + Amount::from_sats(1000)
    );

    assert_eq!(
        fee_consensus.fee(Amount::from_sats(1000)),
        Amount::from_sats(10) + Amount::from_sats(1000)
    );

    assert_eq!(
        fee_consensus.fee(Amount::from_bitcoins(1)),
        Amount::from_sats(1_000_000) + Amount::from_sats(1000)
    );

    assert_eq!(
        fee_consensus.fee(Amount::from_bitcoins(10_000)),
        Amount::from_bitcoins(100) + Amount::from_sats(1000)
    );
}
...
pub struct WalletClientConfig {
    /// The public keys for the bitcoin multisig
    pub bitcoin_pks: BTreeMap<PeerId, PublicKey>,
    /// Confirmations required for a bitcoin transaction to be confirmed
    pub finality_delay: u64,
    /// The minimum amount a user can send on chain
    pub dust_limit: bitcoin::Amount,
    /// Fees taken by the guardians to process wallet inputs and outputs

...
use fedimint_core::module::{
    api_endpoint, Amounts, ApiEndpoint, ApiVersion, CoreConsensusVersion, InputMeta,
...
use fedimint_walletv2_common::config::{
    WalletClientConfig, WalletConfig, WalletConfigPrivate, WalletGenParams,
};
    ...
    async fn consensus_proposal<'a>(
        ...
        if let Some(status) = self.btc_rpc.status() {
            ...
            items.push(WalletConsensusItem::BlockCount(
                status.block_count.saturating_sub(self.cfg.consensus.finality_delay),
            ));

            items.push(WalletConsensusItem::Feerate(Some(status.fee_rate.sats_per_kvb)));
        } else {

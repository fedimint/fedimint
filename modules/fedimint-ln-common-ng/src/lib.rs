...
use crate::db::{IncomingContractKey, OutgoingContractKey};

pub const KIND: ModuleKind = ModuleKind::from_static_str("ln_ng");
const CONSENSUS_VERSION: ModuleConsensusVersion = ModuleConsensusVersion { major: 0, minor: 0 };

#[derive(Debug, Clone, Eq, PartialEq, Hash, Deserialize, Serialize, Encodable, Decodable)]
pub enum Witness {
    Outgoing(OutgoingContractKey, OutgoingWitness),
    Incoming(IncomingContractKey, AggregateDecryptionKey),
}
...
pub enum LightningInputError {
    #[error("Contract already claimed with preimage {0:?}")]
    AlreadyClaimed([u8; 32]),
    #[error("No contract found for given ContractId")]
    UnknownContract,
    #[error("The input's amount does not match the contract's amount")]
    InvalidAmount,

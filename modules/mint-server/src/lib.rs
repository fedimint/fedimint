...
use fedimint_api::config::{scalar, DkgRunner};
...
use fedimint_mint_common::{
    ...
    MintPendingOutput, MintSpendableOutput, MINT_MODULE_KEY,
};
...
impl ServerModulePlugin for Mint {
    ...
    async fn await_consensus_proposal<'a>(&'a self) {
        if self.consensus_proposal(rand::rngs::OsRng).await.is_empty() {
    ...
    fn build_verification_cache<'a>(
        ...
        MintVerificationCache { valid_coins }
    }
...
mod test {
    ...
    fn build_configs() -> (Vec<MintConfig>, serde_json::Value) {

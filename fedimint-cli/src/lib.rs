    ...
    async fn load_database(&self) -> CliResult<Database> {
        ...
        match self.db_backend {
            DatabaseBackend::Fjall => {
                debug!(target: LOG_CLIENT, "Using Fjall database backend");
                Ok(fedimint_fjall::FjallDb::open(&db_path)
                    .map_err_cli_msg("could not open fjall database")?
                    .into())
            }

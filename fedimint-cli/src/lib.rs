...
impl FedimintCli {
    ...
    async fn client_open(&self, cli: &Opts) -> CliResult<ClientHandleArc> {
        ...
        } else if let Some(stored_creds) =
            load_admin_creds(&mut db.begin_transaction_nc().await).await
        {
    ...
    async fn handle_command(&mut self, cli: Opts) -> CliOutputResult {
        match cli.command.clone() {
            ...
            Command::Admin(AdminCmd::Auth {
                ...
                if !force {
                    let existing = load_admin_creds(&mut db.begin_transaction_nc().await).await;
                    if existing.is_some() {
                        return Err(CliError {

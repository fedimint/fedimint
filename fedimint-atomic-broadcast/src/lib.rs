...
//!   it and sends the resulting serialization it to its running atomic
//!   broadcast instance via the mempool item sender.
...
//! * The unit is then included in a [Message] and send to the server via the
//!   outgoing message sender.
//! * The server receives the message, serializes it via consensus_encode and
//!   sends it to its peers, which in turn deserialize it via consensus_decode
//!   and relay it to their broadcast instance via their incoming message
//!   sender.
...
/// An accepted item is created and recorded in the corresponding block if and
/// only if  Fedimint Consensus decides to accept an [OrderedItem].

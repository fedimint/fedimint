...
use fedimint_api_client::api::EndpointRegistry;
...
impl GatewayClientBuilder {
    pub async fn new(
        ...
        Ok(Self {
            endpoints: EndpointRegistry::build_from_server_env()?.bind().await?,
            work_dir,
            registry,
            db_backend,
        })
    ...
    pub async fn recover(
        ...
        let client = client_builder
            .preview(self.endpoints.clone(), &config.invite_code)
            .await?
    ...
    pub async fn build(
        ...
        if Client::is_initialized(&db).await {
            client_builder
                .open(self.endpoints.clone(), db, root_secret)
                .await
        } else {
            client_builder
                .preview(self.endpoints.clone(), &invite_code)
                .await?
                .join(db, root_secret)
                .await
        }

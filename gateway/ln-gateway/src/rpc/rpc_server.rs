...
use fedimint_ln_client_ng::{CreateInvoicePayload, SendPaymentPayload};
...
fn v1_routes(gateway: Gateway) -> Router {
    ...
    let public_routes = Router::new()
        ...
        .route("/payment_info", post(payment_info_ng))
        .route("/send_payment", post(send_payment_ng))
        .route("/create_invoice", post(create_invoice_ng));
...
async fn payment_info_ng(
    Extension(gateway): Extension<Gateway>,
    Json(federation_id): Json<FederationId>,
) -> Json<Value> {
    Json(json!(gateway.payment_info(&federation_id).await))
}

async fn send_payment_ng(
    Extension(gateway): Extension<Gateway>,

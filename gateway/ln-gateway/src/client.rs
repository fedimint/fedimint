...
impl GatewayClientBuilder {
    pub async fn build(
        ...
        let client_secret =
            match Client::load_decodable_client_secret::<[u8; 64]>(client_builder.db_no_encoders())
                .await
            {
                Ok(secret) => secret,
                Err(_) => {
                    ...
                    Client::store_encodable_client_secret(client_builder.db_no_encoders(), secret)
                        .await
        ...
        if Client::is_initialized(client_builder.db_no_encoders()).await {
            client_builder

...
use fedimint_ln_common_ng::{LightningInput, LightningOutputOutcome, Witness};
    ...
    fn transitions(
        ...
        match &self.state {
            ReceiveSMState::Funding(..) => {
                vec![
                    StateTransition::new(
                        Self::await_funding_rejected(global_context.clone(), self.out_point.txid),
...
impl ReceiveStateMachine {
    ...
    async fn transition_funding_rejected(
        ...
        ReceiveStateMachine {
            ...
            out_point: old_state.out_point,
            state: ReceiveSMState::Rejected(error),
        }
    }
    ...
    async fn await_outcome_ready(
        ...
        loop {
            ...
            let pks = tpe_pks.clone();
            let contract = decryption_contract.clone();
                            ...
                            match outcome {
                                LightningOutputOutcome::Incoming(share) => {
                                    if !contract.verify_decryption_share(
                                        pks.get(&peer).ok_or(anyhow!("Unknown peer"))?,
                                        &share,
                                    ) {
    ...
    async fn transition_outcome_ready(
        ...
        let refund_keypair = match old_state.state {
            ReceiveSMState::Funding(keypair) => keypair,
            ...
            .verify_agg_decryption_key(&tpe_agg_pk, &agg_decryption_key)
        {
            ...
            return ReceiveStateMachine {
                ...
                contract: old_state.contract,
                out_point: old_state.out_point,
                state: ReceiveSMState::Failure,
            };
        }

        if let Some(preimage) = old_state.contract.decrypt_preimage(&agg_decryption_key) {
            return ReceiveStateMachine {
                operation_id: old_state.operation_id,
                contract: old_state.contract,
                out_point: old_state.out_point,
                state: ReceiveSMState::Success(preimage),
            };
        }
        ...
        let client_input = ClientInput::<LightningInput, LightningClientStateMachines> {
            input: LightningInput {
                ...
                witness: Witness::Incoming(old_state.contract.contract_key(), agg_decryption_key),
            },
            keys: vec![refund_keypair],
        ...
        ReceiveStateMachine {
            ...
            contract: old_state.contract,
            out_point: old_state.out_point,
            state: ReceiveSMState::Refunding(out_points),
        }
    }
}

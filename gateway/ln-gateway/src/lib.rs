...
impl Gateway {
    ...
    async fn try_handle_lightning_payment_ln_legacy(
        ...
        // Just forward the payment if we do not have a federation that
    ...
    pub async fn handle_shutdown_msg(&self, task_group: TaskGroup) -> AdminResult<()> {
        ...
        // Take the current state, replacing it with a dummy variant.
        // We'll restore the original state if the shutdown process fails.
        // This is safe since we have the write lock on the state.
        let old_state = std::mem::replace(&mut *state_guard, GatewayState::Disconnected);

        if let GatewayState::Running { lightning_context } = old_state {
            // Replace the state with `ShuttingDown`, reusing the `lightning_context`
            *state_guard = GatewayState::ShuttingDown { lightning_context };
            ...
            // If the state was not `Running`, restore the original state
            *state_guard = old_state;
        }

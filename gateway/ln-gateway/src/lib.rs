...
impl Gateway {
    ...
    pub async fn handle_withdraw_msg(
        ...
        let address = if let Some(gateway_config) = self.gateway_config.read().await.as_ref() {
            let network = gateway_config.network.0;
            if let Ok(address) = address.require_network(network) {
                address
            } else {
                return Err(AdminGatewayError::WithdrawError {
                    failure_reason: format!("Gateway is running on network {network}, but provided withdraw address is for network {address_network}")
                });
            }
        } else {
            return Err(AdminGatewayError::GatewayConfigurationError(
                "Gateway config must be set to perform withdrawals".to_string(),
            ));
        };
        ...
        let (amount, fees) = match amount {
            ...
            BitcoinAmountOrAll::All => {
                ...
                let fees = wallet_module
                    .get_withdraw_fees(address.as_unchecked().clone(), balance)
                    .await?;
            ...
            BitcoinAmountOrAll::Amount(amount) => (
                amount,
                wallet_module
                    .get_withdraw_fees(address.as_unchecked().clone(), amount)
                    .await?,
            ),
        };

        let operation_id = wallet_module
            .withdraw(address.as_unchecked().clone(), amount, fees, ())
            .await?;
    ...
    pub async fn handle_get_ln_onchain_address_msg(&self) -> AdminResult<Address> {
        ...
        let network = if let Some(gateway_config) = self.gateway_config.read().await.as_ref() {
            gateway_config.network.0
        } else {
            return Err(AdminGatewayError::GatewayConfigurationError(
                "Gateway config must be set to get onchain address".to_string(),
            ));
        };

        let unchecked_address = Address::from_str(&response.address).map_err(|e| {
            AdminGatewayError::Lightning(LightningRpcError::InvalidMetadata {
                failure_reason: e.to_string(),
            })
        })?;

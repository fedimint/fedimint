...
impl GatewayLightning for ClnRpcService {
    ...
    async fn wait_for_chain_sync(
        ...
        for _ in 0..max_retries {
            let (blockheight, warning_bitcoind_sync) = self
                .rpc_client()
                .await
                .map_err(|e| Status::internal(e.to_string()))?
                .call(cln_rpc::Request::Getinfo(
                ...
                .map(|response| match response {
                    cln_rpc::Response::Getinfo(model::responses::GetinfoResponse {
                        ...
                        ..
                    }) => Ok((blockheight, warning_bitcoind_sync)),
                    _ => Err(ClnExtensionError::RpcWrongResponse),
                })
                .map_err(|e| {
            ...
            if blockheight >= target_block_height && warning_bitcoind_sync.is_none() {
                return Ok(tonic::Response::new(EmptyResponse {}));
            }

            if let Some(retry_delay) = retry_delay.clone() {
                sleep(retry_delay.try_into().map_err(|e: DurationError| {
                    Status::internal(format!(
                        "Failed to convert prost Duration to std Duration: {}",
                        e
                    ))
                })?)
                .await;
